...
exports.nodeView = function(req, res, next){
    var nodeId = req.params.id;
    GizmoCore.modules.node._loadView(nodeId);

    return true;
}

exports._loadView = function(nid, alias){
    ...
    if (!nid && !alias){
        ...
        GizmoCore.Schema.Node.findOne({ $or : [{alias : alias }, {_id : nid }] }, function(err, node){
            if (!err && node){
                GizmoCore.invoke('setTitle','theme',node.title);
                GizmoCore.render('node/node-view', {node : node});
            }else {
                GizmoCore.render('error', {error : {title : 'Error 404' , message : 'Page not found.'}});
            }
        });
